  ...
  return (
    <div className="glass-card p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button className="flex items-center space-x-2 text-xs font-medium text-slate-400 hover:text-slate-200 border border-slate-800 px-3 py-1.5 rounded-lg bg-slate-900/60">
      ...
      <div className="space-y-2">
        {logs.map((log, idx) => (
          <div key={idx} className="p-3 rounded-xl bg-slate-950/60 border border-slate-800/80 text-xs flex items-center justify-between">
            <div>
              <span className="font-mono text-indigo-400 font-medium">[{log.agent}]</span>{' '}
