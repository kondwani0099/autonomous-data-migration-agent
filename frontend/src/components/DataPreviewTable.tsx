    ...
    <div className="glass-card p-6 space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex items-center space-x-2 text-xs text-amber-400 bg-amber-500/10 px-3 py-1.5 rounded-lg border border-amber-500/20">
          ...
          <tbody className="divide-y divide-slate-800/60 bg-slate-950/40">
            {preview.sample_records.map((row, idx) => (
              <tr key={idx} className="hover:bg-slate-900/60 transition-colors">
                <td className="px-4 py-3 font-mono text-slate-400">{String(row.sale_date || '')}</td>
                <td className="px-4 py-3 font-medium text-slate-200">{String(row.customer_name || '')}</td>
                ...
                <td className="px-4 py-3 font-semibold text-indigo-400">${Number(row.total_amount || 0).toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
