      ...
      <div className="space-y-3">
        {entries.length === 0 ? (
          ...
          entries.map((entry, idx) => (
            <div key={idx} className="flex items-start space-x-3 p-3 rounded-xl bg-slate-900/40 border border-slate-800 text-xs">
              <ChevronRight className="w-4 h-4 text-indigo-400 mt-0.5 flex-shrink-0" />
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  <span className="font-semibold text-indigo-300">{entry.agent}</span>
